...
def sort_mails():
    ...
    for file_path in sorted(INBOX_DIR.iterdir()):
        ...
        try:
            ...
            move_result = file_manager.move_file_to_category(file_path, classification_result)
            category = classification_result.get("category", "other")
            category_counter[category] = category_counter.get(category, 0) + 1

            if move_result["success"]:
                sorted_files += 1
                print(f'{file_path.name} -> {classification_result["folder"]} [{classification_result["category"]}]')
            else:
                error_files += 1
    ...
    print_summary(total_files, sorted_files, unreadable_files, error_files, category_counter)
